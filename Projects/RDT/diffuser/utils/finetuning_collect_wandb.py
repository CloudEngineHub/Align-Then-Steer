...
class Finetuner(object):
    ...
    def train_policy_func(self, state_dict, expert_dict, count, policy_steps):
        ...
        with torch.no_grad():
            ...
            if expert_dict["state"].shape[0] > self.p_batch_size:
    ...
    def finetune(self):
        ...
        env_dict = {"observations": list(), "actions": list(), "rewards": list(), "terminals": list()}
        self.eval()
        policy_steps = self.gradient_accumulate_every * self.p_step
        obs = self.train_env.reset()[0][None]
        env_dict["observations"].append(obs.squeeze(0))
        obs_history = []
        done = False
        success = False
        flag = True
        ...
        while self.train_step < self.finetune_steps:
            ...
            with torch.no_grad():
                ...
                for step in range(start, end):
                    ...
                    env_dict["actions"].append(action)
                    env_dict["rewards"].append(np.float32(reward)[None])
                    env_dict["terminals"].append(np.float32(done)[None])
                    if not done:
                        env_dict["observations"].append(obs)
            ...
            if success_episode < self.init_episode and self.global_step > 5e5:
                success_episode = self.init_episode + 1
            # if (done and success):
            if success_episode < self.init_episode:
            ...
            if success_episode < self.init_episode:
                ...
                train_flag = done and (state_dict["state"].shape[0] > self.p_batch_size)
            if train_flag:
            ...
            if done:
                if success_episode >= self.init_episode:
                    episode_data = {"observations": np.array, "actions": np.array, "rewards": np.array,
                                    "terminals": np.array}
                    for key, value in env_dict.items():
                        value = np.array(value)
                        stack_value = np.stack(value)
                        episode_data[key] = stack_value
                    os.makedirs(f"../ft_data/{self.task}", exist_ok=True)
                    np.savez(f"../ft_data/{self.task}/{1000 + real_episode}.npz", **episode_data)
                env_dict = {"observations": list(), "actions": list(), "rewards": list(), "terminals": list()}
                temp_dict = {"state": torch.FloatTensor(), "next_state": torch.FloatTensor(),
                             "timestep": torch.LongTensor(), "final_reward": torch.FloatTensor(),
                             "log_prob": torch.FloatTensor(), "x0": torch.FloatTensor(), "cond": torch.FloatTensor()}

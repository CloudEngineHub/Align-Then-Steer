...
from models.rdt_runner_ATE import RDTRunner
...
def freezeVAE(LoraRunner):
    for name, param in LoraRunner.named_parameters():
        if 'VAEmodel' in name:
...
def train(args, logger):
    ...
    if (
        ...
        rdt = RDTRunner.from_pretrained(args.pretrained_model_name_or_path, vae_config=config["model"]["vae"])
    else:
    ...
    cprint("Freezing VAE...", "green")
    freezeVAE(rdt)
    cprint("VAE Frozen...", "green")

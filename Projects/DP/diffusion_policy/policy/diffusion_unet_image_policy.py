...
from diffusion_policy.model.vae.latent_guidance import LatentGuidence
from diffusion_policy.common.pytorch_util import dict_apply


## Prepared for guidance calculation
class ModifiedDDPMSchduler(DDPMScheduler):
    def add_noise(
        self,
...
class DiffusionUnetImagePolicy(BaseImagePolicy):
    ...
    def __init__(
        self,
        shape_meta: dict,
        noise_scheduler: ModifiedDDPMSchduler,
        ...
        **kwargs,
        ...
        if vae_config is not None:
            self.guidence = LatentGuidence(vae_config)
        else:
            self.guidance = None
    ...
    def compute_loss(self, batch):
        ...
        noisy_trajectory, alpha_t_bars = self.noise_scheduler.add_noise(trajectory, noise, timesteps)
        ...
        if self.guidance is not None:
            # incorporate latent guidance to calibrate score matching
            loss = self.guidance(alpha_t_bars, pred, target, noisy_trajectory, pred_type, "none")
            loss = loss * loss_mask.type(loss.dtype)
            loss = reduce(loss, "b ... -> b (...)", "mean")
            loss = loss.mean()

        else:
            # original implementation of diffusion policy
